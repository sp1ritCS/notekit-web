...
function encoded_string_to_bytes(encoded) {
...
function encoded_string_to_stream(encoded) {
	const bytes = encoded_string_to_bytes(encoded);
...
async function notekit_drawing_from_string_native(encoded) {
...
async function notekit_drawing_from_string_fallback(encoded) {
	return new NoteKitDrawing(new Uint32Array(
		pako.inflate(encoded_string_to_bytes(encoded)).buffer
	));
}

var notekit_drawing_from_string = () => { return new Promise((resolve, reject) => reject("Not yet initialized.")); }

function notekit_init(callback) {
	notekit_drawing_from_string_native("eJxjYEAAAAAMAAE=")
		.then(_drawing => {
			console.log("Properly initialized, using native stategy.");
			notekit_drawing_from_string = notekit_drawing_from_string_native;
			callback("native");
		})
		.catch(e => {
			console.log(`Error ${e}, falling back to fallback stategy.`);

			const body = document.getElementsByTagName('body')[0];
			const script = document.createElement('script');
			script.type = 'text/javascript';
			script.src = "./pako_inflate.js";

			const pako_loaded_callback = () => {
				console.log("Pako was loaded, using fallback staregy.");
				notekit_drawing_from_string = notekit_drawing_from_string_fallback;
				callback("fallback:pako");
			};

			script.onreadystatechange = pako_loaded_callback;
			script.onload = pako_loaded_callback;
